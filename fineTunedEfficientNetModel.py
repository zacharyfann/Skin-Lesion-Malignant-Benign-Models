...
for layer in model.layers[:int(len(model.layers) * 0.80)]:
...
train_df, val_df = train_test_split(df, test_size=0.2, random_state=77)
...
lr_schedule = schedules.ExponentialDecay(
    initial_learning_rate=1e-5,  # Lower learning rate for better fine-tuning
    decay_steps=1000,
    decay_rate=0.96,
    staircase=True
)
...
# Callbacks
checkpoint = ModelCheckpoint('efficientNet_model_finetuned.keras', monitor='val_accuracy', save_best_only=True, mode='max')
...
history = model.fit(
    ...
    epochs=25,  # Fine-tuning with slightly fewer epochs
...
model.save('efficientNet_model_finetuned.keras')
